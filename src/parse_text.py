...
def main():
    filename = 'data/general-conference-talk-references.txt'
    rawtext = read_in_raw_text(filename)
...
def extract_text_to_dataframe(rawtext: str) -> pd.DataFrame:
    ...
    df = pd.DataFrame(raw_column_data, columns=['raw_scripture', 'url', 'raw_talk_reference', 'talk_title'])
...
df['num_hyphens'] = df.scripture.str.count('-')
df['num_commas'] = df.scripture.str.count(',')
df['num_colons'] = df.scripture.str.count(':')

df['citation_nums'] = df.scripture.str.split().str[-1]
