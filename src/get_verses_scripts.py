...
def main():
    ...
    with open(chapters_scripts_file, encoding='utf-8') as f:
        book_scripts = [x for x in f.read().split('\n') if x]

    with open(output_file, 'w', encoding='utf-8') as f:
        for x in book_scripts:
            nargs = len(x.split(','))
            if (nargs < 4) & ('getFilter' in x):
                print(x)
                browser.execute_script(x)
                time.sleep(1)
                soup = get_citationindex_dynamic(browser)
                verse_scripts = [x.get('onclick') for x in soup.find_all('a') if x.find('div')]

                for y in verse_scripts:
                    f.write(f'{y}\n')
